...
const ECHO_MG_ASSETS = Object.freeze({
  ...
  ch5: {
    ...
    evidenceBoard: 'img/scenes/ch5_evidence_board.webp',
    ...
    linkReference: 'img/scenes/ch5_echo_relation_tree.webp',
...
function runGraphicMapInvestigation() {
  return new Promise((resolve) => {
    ...
    const viewed = {};
    const points = [
      { id: 'entrance', label: ECHO_I18N.t('A入口'), x: 20, y: 72, thumb: 'static', note: ECHO_I18N.t('入口動線正常。雨水積在階梯下方。') },
      { id: 'exit', label: ECHO_I18N.t('B出口'), x: 79, y: 22, thumb: 'static', note: ECHO_I18N.t('出口沒有異常通行紀錄。') },
      { id: 'camera', label: 'CAM 07', x: 63, y: 48, thumb: 'cctv', note: ECHO_I18N.t('監視器拍得到最後畫面，但 03:17 後訊號中斷。') },
      { id: 'blind', label: ECHO_I18N.t('死角'), x: 39, y: 33, thumb: 'static', note: ECHO_I18N.t('這個位置被柱體遮蔽，監視器拍不到。') },
      { id: 'graffiti', label: ECHO_I18N.t('塗鴉牆'), x: 48, y: 78, thumb: 'static', note: ECHO_I18N.t('牆面有一段模糊字樣。再點一次放大。') }
    ];

    const root = echoMiniGameShell('CH2-1 · UNDERGROUND MAP', ECHO_I18N.t('萬華地下道調查'), ECHO_I18N.t('點擊圖上節點調查。塗鴉牆需要二次確認。'), ECHO_MG_ASSETS.ch21.panel);
    ...
    stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch21.base, 'echo-mg-stage-img', ECHO_I18N.t('地下道地圖')));
    stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch21.markers, 'echo-mg-marker-sheet', ECHO_I18N.t('地圖標記素材')));
    ...
    note.textContent = ECHO_I18N.t('選擇一個調查點位。');
    const done = document.createElement('button');
    done.type = 'button';
    done.className = 'echo-mg-primary';
    done.textContent = ECHO_I18N.t('完成調查');
    done.disabled = true;
...
function runSpotDifference() {
  return new Promise((resolve) => {
    ...
    const hotspots = [
      { id: 'photos', label: ECHO_I18N.t('照片配置'),   x: 58.6, y: 26.3, hitW: 74, hitH: 54, reaction: ECHO_I18N.t('我不太喜歡拍自己。可是有些照片……我又捨不得丟。') },
      { id: 'clock',  label: ECHO_I18N.t('時鐘時間'),   x: 68.5, y: 54.7, hitW: 28, hitH: 22, reaction: ECHO_I18N.t('那天我回家很晚……可是我不記得，中間那段時間發生了什麼。') },
      { id: 'note',   label: ECHO_I18N.t('粉紅便條'),   x: 80.5, y: 57.5, hitW: 26, hitH: 24, reaction: ECHO_I18N.t('我有時候會留紙條，怕自己忘記事情。') },
      { id: 'aroma',  label: ECHO_I18N.t('右側物件'),   x: 88.7, y: 58.6, hitW: 26, hitH: 52, reaction: ECHO_I18N.t('我不太喜歡房間太暗。尤其下雨的晚上。') },
      { id: 'teddy',  label: ECHO_I18N.t('小熊消失'),   x: 45.2, y: 60.5, hitW: 46, hitH: 44, reaction: ECHO_I18N.t('那隻熊我一直放在床上。它不應該突然不見。') },
      { id: 'bed',    label: ECHO_I18N.t('床單／毯子變化'), x: 24.6, y: 78.6, hitW: 86, hitH: 58, reaction: ECHO_I18N.t('我其實不太會弄亂床。除非……那天真的很累。') }
    ];
    const totalDifferences = hotspots.length;

    const root = echoMiniGameShell('CH2-2 · SPOT THE DIFFERENCE', ECHO_I18N.t('房間照片比對'), ECHO_I18N.t('上下兩張照片有多處不同。請把所有明顯差異都圈出來。'), ECHO_MG_ASSETS.ch22.frame);
    const body = root.querySelector('.echo-mg-body');
    const foot = root.querySelector('.echo-mg-foot');
    ...
    counter.textContent = ECHO_I18N.t('已找到 0 / ') + totalDifferences + ECHO_I18N.t(' · 上下比對後直接點擊差異');
    const reaction = document.createElement('div');
    reaction.className = 'echo-mg-note spot-reaction';
    reaction.textContent = ECHO_I18N.t('林雨晴：我整理東西的時候……發現這兩張照片好像不太一樣。');
    ...
    function markFound(h) {
      ...
      counter.textContent = ECHO_I18N.t('已找到 ') + found.size + ' / ' + totalDifferences + ' · ' + h.label;
      reaction.textContent = ECHO_I18N.t('林雨晴：') + h.reaction;

      if (found.size === totalDifferences) {
        settled = true;
        const lowerStage = pair.querySelector('.spot-panel-b .spot-stage');
        const complete = echoArtImg(ECHO_MG_ASSETS.ch22.complete, 'echo-mg-complete-art spot-complete-art', ECHO_I18N.t('比對完成'));
    ...
    function makePanel(kind, label) {
      ...
      stage.appendChild(echoArtImg(src, 'echo-mg-stage-img spot-master', ECHO_I18N.t('房間照片 ') + kind.toUpperCase()));
      ...
      hotspots.forEach((h) => {
        ...
        hit.setAttribute('aria-label', kind.toUpperCase() + ECHO_I18N.t(' 圖差異：') + h.label);
...
function runMemoryRepair() {
  return new Promise((resolve) => {
    ...
    const fragments = [
      { id: 0, text: ECHO_I18N.t('K：Oracle 裡到底發生什麼？') },
      { id: 1, text: ECHO_I18N.t('林雨晴：那不是普通專案。') },
      { id: 2, text: ECHO_I18N.t('K：妳說的那個代號……') },
      { id: 3, text: ECHO_I18N.t('林雨晴：你不要直接打名字。') },
      { id: 4, text: ECHO_I18N.t('K：妳之前說過。它和陪伴系統有關。') },
      { id: 5, text: ECHO_I18N.t('林雨晴：……因為它本來就不該公開。') },
      { id: 6, text: ECHO_I18N.t('K：那為什麼還要繼續？') },
      { id: 7, text: ECHO_I18N.t('K：我不是第一次看到妳去那裡。') },
      { id: 8, text: ECHO_I18N.t('林雨晴：那你為什麼還聯絡他？') }
    ];

    const root = echoMiniGameShell('CH3-1 · MEMORY REPAIR', ECHO_I18N.t('記憶修復'), '', ECHO_MG_ASSETS.ch31.board);
    ...
    submit.textContent = ECHO_I18N.t('判定');
    ...
    submit.onclick = () => {
      ...
      if (a === targetIds.length) {
        ...
        const complete = echoArtImg(ECHO_MG_ASSETS.ch31.complete, 'echo-mg-complete-art', ECHO_I18N.t('記憶修復完成'));
...
function runOnlineModeratorGame() {
  return new Promise((resolve) => {
    ...
    const nightLabels = [ECHO_I18N.t('第一夜'), ECHO_I18N.t('第二夜'), ECHO_I18N.t('最終夜')];
    ...
    const players = [
      {
        id: 'unknown17',
        name: ECHO_I18N.t('阿澤'),
        meta: ECHO_I18N.t('15歲 · 在線 127 天'),
        relation: ECHO_I18N.t('林薇的表弟 · 同一邀請連結'),
        portrait: 'img/ui/ch32/ch32_a1.jpg?v=20260905-2',
        statements: [
          ECHO_I18N.t('林薇是我表姊。昨天她還傳訊息叫我不要回這個聊天室。'),
          ECHO_I18N.t('剛才陳默離線，我這邊只看到 OFFLINE，沒有其他狀態。'),
          ECHO_I18N.t('高翔離線後我也只看到 ONLINE 4。誰看得到別的欄位？')
        ]
      },
      {
        id: 'sleepmode',
        name: ECHO_I18N.t('林薇'),
        meta: ECHO_I18N.t('28歲 · 在線 4 年'),
        relation: ECHO_I18N.t('阿澤的表姊 · 三年未見'),
        portrait: 'img/ui/ch32/ch32_b1.jpg?v=20260905-2',
        statements: [
          ECHO_I18N.t('阿澤是我表弟，但我三年沒見他。昨天那則訊息不是我發的。'),
          ECHO_I18N.t('高翔說是陳默邀他進來，可陳默離線前跟我說，他沒邀任何人。'),
          ECHO_I18N.t('我只看得到 ONLINE 4。ACTIVE、RETAINED 這些字，我的畫面沒有。')
        ]
      },
      {
        id: 'lastseen404',
        name: ECHO_I18N.t('陳默'),
        meta: ECHO_I18N.t('42歲 · 最後離線 404 天前'),
        relation: ECHO_I18N.t('高翔的前同事 · 曾轉傳聊天室連結'),
        portrait: 'img/ui/ch32/ch32_c1.jpg?v=20260905-2',
        statements: [
          ECHO_I18N.t('高翔以前和我同公司。這個聊天室連結，我只轉給他看過一次。'),
          ECHO_I18N.t('（已離線）'),
          ECHO_I18N.t('（已離線）')
        ]
      },
      {
        id: 'silentroom',
        name: ECHO_I18N.t('小葵'),
        meta: ECHO_I18N.t('19歲 · 在線未知'),
        relation: ECHO_I18N.t('周伯的鄰居 · 曾幫他設定手機'),
        portrait: 'img/ui/ch32/ch32_a2.jpg?v=20260905-2',
        statements: [
          ECHO_I18N.t('周伯住我家樓上，我幫他調過手機。他不像會自己進這種聊天室的人。'),
          ECHO_I18N.t('陳默的 LOGOUT REQUEST 沒完成。他只是從 ACTIVE 名單被移除了。'),
          ECHO_I18N.t('現在是 ACTIVE 3、RETAINED 1。高翔的離線請求已經完成。')
        ]
      },
      {
        id: 'echo_guest',
        name: ECHO_I18N.t('高翔'),
        meta: ECHO_I18N.t('35歲 · 在線 18 分鐘'),
        relation: ECHO_I18N.t('陳默的前同事 · 由陳默傳入連結'),
        portrait: 'img/ui/ch32/ch32_b2.jpg?v=20260905-2',
        statements: [
          ECHO_I18N.t('陳默是我以前的同事。就是他把這個聊天室連結傳給我。'),
          ECHO_I18N.t('陳默離線後，我只看到 OFFLINE。小葵說的 LOGOUT REQUEST 是什麼？'),
          ECHO_I18N.t('（已離線）')
        ]
      },
      {
        id: '0317',
        name: ECHO_I18N.t('周伯'),
        meta: ECHO_I18N.t('80歲 · 狀態：在線中'),
        relation: ECHO_I18N.t('小葵的樓上鄰居 · 手機曾由小葵設定'),
        portrait: 'img/ui/ch32/ch32_c2.jpg?v=20260905-2',
        statements: [
          ECHO_I18N.t('小葵住樓下。我的手機是她幫忙弄的，這個聊天室是自己跳出來的。'),
          ECHO_I18N.t('我也只看到陳默變成 OFFLINE。什麼叫 LOGOUT REQUEST？'),
          ECHO_I18N.t('我的畫面只有 ONLINE。ACTIVE 跟 RETAINED，我完全沒看過。')
        ]
      }
    ];

    const root = echoMiniGameShell('CH3-2 · ONLINE GAME', ECHO_I18N.t('在線局'), '', ECHO_MG_ASSETS.ch32.panel);
    ...
    relationBoard.innerHTML = ECHO_I18N.t('<b>已知關係</b><span>A1 阿澤 ↔ B1 林薇｜表姊弟</span><span>C1 陳默 ↔ B2 高翔｜前同事</span><span>A2 小葵 ↔ C2 周伯｜鄰居</span>');
    ...
    function latestStatement(player) {
      ...
      if (wentOffline && round > wentOffline) return ECHO_I18N.t('（已離線，沒有新的證詞。）');
    ...
    function showDetail(player) {
      ...
      relation.textContent = ECHO_I18N.t('關係：') + player.relation;
    ...
    function makePortrait(player, index, mode) {
      ...
      photo.alt = player.name + ECHO_I18N.t(' 大頭照');
      return wrap;
    }
    ...
    function makeDiscussionCard(player, index) {
      ...
      mark.setAttribute('aria-label', ECHO_I18N.t('標記 ') + player.name + ECHO_I18N.t(' 的懷疑程度'));
    ...
    function renderDiscussion(message) {
      ...
      phaseLabel.textContent = nightLabels[round - 1] + ECHO_I18N.t(' · 討論');
      systemLine.textContent = message || ECHO_I18N.t('在線證詞已更新');
      roster.innerHTML = '';
      ...
      action.textContent = round < 3 ? ECHO_I18N.t('結束討論') : ECHO_I18N.t('進入最終投票');
    }
    ...
    function triggerLogout() {
      ...
      status.textContent = 'ONLINE ' + onlineCount() + ' · R' + round + '/3';
      phaseLabel.textContent = nightLabels[round - 1] + ECHO_I18N.t(' · 夜間離線');
      systemLine.textContent = player.name + ECHO_I18N.t(' 已離線');
      detail.innerHTML = '<b>SYSTEM</b><span>' + player.name + ECHO_I18N.t(' 已離線。這裡的淘汰只會把名字變灰。</span>');
      ...
      setTimeout(() => {
        ...
        const nextMessage = round === 2
          ? ECHO_I18N.t('比對重點：普通成員只看得到 ONLINE / OFFLINE')
          : ECHO_I18N.t('最後線索：誰說出了未公開的 ACTIVE / RETAINED / LOGOUT REQUEST？');
    ...
    function renderVote() {
      ...
      phaseLabel.textContent = ECHO_I18N.t('最終夜 · 指認');
      systemLine.textContent = ECHO_I18N.t('管理權限持有者只可指認一次');
      detail.innerHTML = ECHO_I18N.t('<b>管理權限</b><span>所有曾在線的人都仍是候選。提示：普通成員只看得到 ONLINE / OFFLINE；能直接讀到 ACTIVE、RETAINED 或 LOGOUT REQUEST 的人最可疑。</span>');
      roster.innerHTML = '';
      ...
      players.forEach((player, index) => {
        ...
        mark.textContent = player.relation + ECHO_I18N.t(' · 標記 ') + suspicionText(player.id);
        ...
        candidate.onclick = () => {
          ...
          detail.innerHTML = ECHO_I18N.t('<b>指認：') + player.name + '</b><span>' + player.statements[2] + '</span>';
          action.disabled = false;
        };
        roster.appendChild(candidate);
      });

      action.textContent = ECHO_I18N.t('確認指認');
      action.disabled = true;
    }
    ...
    function finishVote() {
      ...
      phaseLabel.textContent = success ? ECHO_I18N.t('管理權限已驗證') : ECHO_I18N.t('在線辨識已結束');
      systemLine.textContent = success ? 'ADMIN LOG ACCESS GRANTED' : ECHO_I18N.t('管理權限未確認');
      detail.innerHTML = success
        ? '<b>' + selected.name + ECHO_I18N.t(' · MODERATOR</b><span>管理紀錄已解鎖。</span>')
        : '<b>' + selected.name + ECHO_I18N.t('</b><span>管理權限驗證失敗。正確身分不會公開。</span>');
      setTimeout(() => echoFinishMiniGame(resolve, {
        completed: true,
        success,
        selectedId,
    ...
    renderDiscussion(ECHO_I18N.t('先看三組已知關係，再比對彼此證詞'));
...
function runAudioVerification() {
  return new Promise((resolve) => {
    ...
    const root = echoMiniGameShell('CH3-3 · AUDIO VERIFICATION', ECHO_I18N.t('聲音驗證'), ECHO_I18N.t('找出三段聲音中不同的一段。'), ECHO_MG_ASSETS.ch33.panel);
    ...
    function playClip(index, choice) {
      ...
      player.onerror = () => {
        if (activeAudio === player) activeAudio = null;
        choice.classList.remove('is-playing');
        copy.textContent = ECHO_I18N.t('音訊載入失敗，請重新播放。');
      };
      player.play().catch(() => {
        if (activeAudio === player) activeAudio = null;
        choice.classList.remove('is-playing');
        copy.textContent = ECHO_I18N.t('請再點一次播放按鈕。');
      });
    }
    ...
    function makeChoice(index) {
      ...
      const wave = echoArtImg(ECHO_MG_ASSETS.ch33.wave, 'audio-choice-wave', ECHO_I18N.t('聲音波形 ') + letters[index]);
      const play = document.createElement('button');
      play.type = 'button';
      play.className = 'audio-choice-play';
      play.setAttribute('aria-label', ECHO_I18N.t('播放聲音 ') + letters[index]);
      ...
      select.textContent = ECHO_I18N.t('選 ') + letters[index];
    ...
    function finish() {
      ...
      body.appendChild(echoArtImg(ECHO_MG_ASSETS.ch33.result, 'echo-mg-complete-art audio-verify-result', ECHO_I18N.t('聲音驗證完成')));
      status.textContent = '5 / 5';
      label.textContent = 'VERIFIED';
      copy.textContent = mistakes === 0 ? ECHO_I18N.t('5 關驗證完成。') : ECHO_I18N.t('5 關驗證完成 · 錯誤 ') + mistakes + ECHO_I18N.t(' 次。');
      setTimeout(() => echoFinishMiniGame(resolve, {
        correct,
        total: rounds.length,
        mistakes,
    ...
    function choose(index, choice) {
      ...
      if (index !== rounds[round].answer) {
        ...
        copy.textContent = ECHO_I18N.t('不一致。再聽一次。');
      ...
      copy.textContent = ECHO_I18N.t('驗證成功。');
      transitionLocked = true;
      setControlsDisabled(true);
...
function runMirrorFragment(round = 1) {
  return new Promise((resolve) => {
    const spec = round === 2 ? {
      ...
      title: ECHO_I18N.t('鏡像碎片 · 深層紀錄'),
      subtitle: ECHO_I18N.t('拖曳更多鏡像碎片，還原受損輸出。'),
      ...
      title: ECHO_I18N.t('鏡像碎片'),
      subtitle: ECHO_I18N.t('拖曳 6 塊鏡像文字碎片，還原受損輸出。'),
      pieces: ['ɘ', 'ɿɘ', 'ʜ', ' m', "'", 'I'],
      decoded: "I'm here",
    ...
    laneLabel.textContent = ECHO_I18N.t('同步紀錄受損 · 請重組鏡像內容');
    ...
    poolLabel.textContent = ECHO_I18N.t('鏡像碎片');
    ...
    verify.textContent = ECHO_I18N.t('重組紀錄');
    verify.disabled = true;

    const hint = document.createElement('button');
    hint.type = 'button';
    hint.className = 'echo-mg-secondary mirror-hint';
    hint.textContent = ECHO_I18N.t('提示一格');
    hint.hidden = true;

    const note = document.createElement('div');
    note.className = 'echo-mg-note mirror-note';
    note.textContent = ECHO_I18N.t('手機操作：點第一塊，再點第二塊可直接交換位置；也可拖曳。');
      ...
      card.onclick = () => {
        ...
        if (selectedCard) {
          if (selectedCard === card) {
            clearSelectedCard();
            note.textContent = ECHO_I18N.t('已取消選取。');
            return;
          }
          swapCards(selectedCard, card);
          clearSelectedCard();
          note.textContent = ECHO_I18N.t('已交換兩塊碎片。可以繼續交換，或按「重組紀錄」檢查。');
          return;
        }
        if (source) {
          selectedCard = card;
          card.classList.add('is-selected');
          note.textContent = ECHO_I18N.t('已選取這一塊。再點另一塊即可交換位置。');
          return;
        }
        const empty = slots.find((slot) => !slotCard(slot));
        if (empty) {
          placeCard(card, empty);
          note.textContent = ECHO_I18N.t('碎片已放入。放滿後可直接點兩塊交換位置。');
        }
      };
      return card;
    }
    ...
    hint.onclick = () => {
      ...
      note.textContent = ECHO_I18N.t('鏡像提示已固定 1 格。目前 ') + correctPositions + ' / ' + spec.pieces.length + ECHO_I18N.t(' 塊位置正確。');
    };
    ...
    function revealInterpretation() {
      ...
      verify.remove();
      note.textContent = ECHO_I18N.t('紀錄修復完成 · 解讀鏡像內容');

      const decoded = document.createElement('div');
      decoded.className = 'mirror-decoded';
      decoded.innerHTML = ECHO_I18N.t('<span>鏡像還原</span><b>') + spec.decoded + '</b>';
      ...
      spec.choices.forEach((text, index) => {
        ...
        button.onclick = () => {
          ...
          if (index !== spec.answer) {
            ...
            note.textContent = ECHO_I18N.t('解讀不一致。鏡像層沒有回應。');
            setTimeout(() => button.classList.remove('is-wrong'), 420);
            return;
          }
          settled = true;
          button.classList.add('is-correct');
          note.textContent = ECHO_I18N.t('紀錄修復完成');
          setTimeout(() => echoFinishMiniGame(resolve, {
            completed: true,
            round,
    ...
    verify.onclick = () => {
      ...
      if (!correct) {
        ...
        hint.hidden = false;
        note.textContent = ECHO_I18N.t('順序不對，但有 ') + correctPositions + ' / ' + spec.pieces.length + ECHO_I18N.t(' 塊已在正確位置。點兩塊可直接交換，或使用提示。');
        return;
      }
...
function runMirrorLock(stage = 1) {
  return new Promise((resolve) => {
    ...
    const root = echoMiniGameShell(
      ...
      stage === 2 ? ECHO_I18N.t('人格覆寫') : ECHO_I18N.t('鏡像先行'),
      stage === 2 ? ECHO_I18N.t('聊天室開始在你之前完成回覆。') : ECHO_I18N.t('觀察鏡像層，然後選擇你的回覆。'),
    ...
    stageEl.appendChild(echoArtImg(ECHO_MG_ASSETS.ch41.screenshot, 'mirror-lock-shot', ECHO_I18N.t('鏡像畫面')));
    stageEl.appendChild(echoArtImg(ECHO_MG_ASSETS.ch41.lock, 'mirror-lock-overlay', ECHO_I18N.t('鏡像鎖定')));
    ...
    if (stage === 1) {
      ...
      note.textContent = ECHO_I18N.t('鏡像輸出正在形成……');
      ...
      const choices = [
        { text: ECHO_I18N.t('只是格式問題'), locked: true },
        { text: ECHO_I18N.t('那不是我'), correct: true },
        { text: ECHO_I18N.t('你覺得是誰') }
      ];
      choices.forEach((choice) => {
        ...
        button.onclick = () => {
          ...
          note.textContent = choice.correct ? ECHO_I18N.t('鏡像層與你的回覆短暫重合。') : ECHO_I18N.t('鏡像層沒有接受這個回覆。');
      ...
      setTimeout(() => {
        ...
        note.textContent = ECHO_I18N.t('你還沒碰到選項，右側已先出現「oИ」。');
      }, 520);
    } else {
      status.textContent = 'OVERRIDE';
      note.textContent = ECHO_I18N.t('不要動。聊天室正在比你更早決定。');
      const rounds = [
        { choices: [ECHO_I18N.t('不是我。'), ECHO_I18N.t('妳先停下來。'), ECHO_I18N.t('……我在。')], forced: 2 },
        { choices: [ECHO_I18N.t('我不接受。'), ECHO_I18N.t('那是鏡子裡的我。'), ECHO_I18N.t('……是我。')], forced: 2 }
      ];
      let index = 0;
      let forcedCount = 0;
      ...
      function renderOverrideRound() {
        ...
        label.textContent = ECHO_I18N.t('覆寫 ') + (index + 1) + ' / ' + rounds.length;
        ...
        current.choices.forEach((text, choiceIndex) => {
          ...
          button.onclick = () => {
            ...
            note.textContent = ECHO_I18N.t('你碰到了「') + text + ECHO_I18N.t('」——但訊息還沒有送出。');
          };
          actions.appendChild(button);
        });
        foot.insertBefore(actions, note);
        ...
        setTimeout(() => {
          ...
          note.textContent = attempt
            ? ECHO_I18N.t('你的選擇被覆寫：') + attempt + ' → ' + forcedText
            : ECHO_I18N.t('你還沒選，聊天室已送出：') + forcedText;
          ...
          setTimeout(() => {
            ...
            if (index >= rounds.length) {
              ...
              note.textContent = ECHO_I18N.t('兩次回覆皆由鏡像層先行完成。');
...
function runEvidenceArchive() {
  return new Promise((resolve) => {
    ...
    const records = [
      ['CAM 07', ECHO_I18N.t('03:17 監視器'), 'img/scenes/tunnel_cctv.jpg', ECHO_I18N.t('地下道畫面出現不合理的人影。')],
      ['RAIN', ECHO_I18N.t('林雨晴最後自拍'), 'img/rain/rain_tunnel.jpg', ECHO_I18N.t('牆上留下「Echo」字樣。')],
      ['ROOM', ECHO_I18N.t('你的房間'), 'img/scenes/room_eva.jpg', ECHO_I18N.t('時間戳比現在晚三分鐘。')],
      ['GROUP', ECHO_I18N.t('記憶群像'), 'img/scenes/ch31_group_photo.jpg', ECHO_I18N.t('EVA 出現在一張不應存在的合照裡。')],
      ['BOARD', ECHO_I18N.t('ECHO 證據板'), ECHO_MG_ASSETS.ch5.evidenceBoard, ECHO_I18N.t('所有零散線索被整理到同一塊證據板上。')]
    ];
    const root = echoMiniGameShell('CH5 · EVIDENCE ARCHIVE', ECHO_I18N.t('證據檔案庫'), ECHO_I18N.t('重新檢查至少 3 份證據。它們會成為 ECHO 關聯圖的節點。'), ECHO_MG_ASSETS.ch5.archivePanel);
    const body = root.querySelector('.echo-mg-body');
    const foot = root.querySelector('.echo-mg-foot');
    ...
    const detailPreview = document.createElement('img');
    detailPreview.className = 'archive-detail-preview';
    detailPreview.alt = ECHO_I18N.t('證據預覽');
    detailPreview.hidden = true;
    const detailCopy = document.createElement('span');
    detailCopy.textContent = ECHO_I18N.t('選擇一份證據。');
    detail.appendChild(detailPreview);
    detail.appendChild(detailCopy);
    const done = document.createElement('button');
    done.type = 'button';
    done.className = 'echo-mg-primary';
    done.textContent = ECHO_I18N.t('完成證據檢查');
    done.disabled = true;
    ...
    records.forEach((r, i) => {
      ...
      card.onclick = () => {
        ...
        detailPreview.src = r[2];
        detailPreview.hidden = false;
        detailCopy.textContent = r[3];
...
function runEchoLinkBoard() {
  return new Promise((resolve) => {
    ...
    const nodes = [
      ['tunnel', ECHO_I18N.t('地下道'), 18, 26],
      ['rain', ECHO_I18N.t('林雨晴'), 78, 24],
      ['voice', ECHO_I18N.t('第三個聲音'), 18, 74],
      ['memory', ECHO_I18N.t('改寫記憶'), 78, 72]
    ];
    const root = echoMiniGameShell('CH5 · ECHO LINK BOARD', ECHO_I18N.t('ECHO 關聯圖'), ECHO_I18N.t('把四個證據節點連到中央 ECHO。'), ECHO_MG_ASSETS.ch5.linkBase);
    const body = root.querySelector('.echo-mg-body');
    const foot = root.querySelector('.echo-mg-foot');
    const stage = document.createElement('div');
    stage.className = 'echo-mg-stage link-stage';
    stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch5.linkBase, 'echo-mg-stage-img', ECHO_I18N.t('ECHO 關聯圖背景')));
    const center = echoArtImg(ECHO_MG_ASSETS.ch5.linkEcho, 'link-echo-node', 'ECHO');
    stage.appendChild(center);
    const note = document.createElement('div');
    note.className = 'echo-mg-note';
    note.textContent = ECHO_I18N.t('已連接 0 / 4');
    ...
    nodes.forEach((n) => {
      ...
      btn.onclick = () => {
        ...
        const glow = echoArtImg(ECHO_MG_ASSETS.ch5.linkGlow, 'link-glow', ECHO_I18N.t('連線'));
        glow.style.left = n[2] + '%';
        glow.style.top = n[3] + '%';
        stage.appendChild(glow);
        note.textContent = ECHO_I18N.t('已連接 ') + connected.size + ' / 4';
        if (connected.size === nodes.length) {
          settled = true;
          note.textContent = ECHO_I18N.t('關聯完成 · ECHO 關聯樹已展開');
          const reference = echoArtImg(ECHO_MG_ASSETS.ch5.linkReference, 'link-reference-complete', ECHO_I18N.t('ECHO 關聯樹狀圖'));
          stage.appendChild(reference);
          setTimeout(() => reference.classList.add('is-visible'), 40);
          setTimeout(() => echoFinishMiniGame(resolve, { connected: connected.size, completed: true }), 1850);
        }
      };
      stage.appendChild(btn);
    });
...
function runResidualVoices() {
  return new Promise((resolve) => {
    ...
    const voices = [
      ['rain', ECHO_I18N.t('林雨晴'), ECHO_I18N.t('「不要跟我說話。那時候的我，可能已經不是我了。」')],
      ['k', 'K', ECHO_I18N.t('「別變成我們。在線太久之後，你會覺得這裡比較舒服。」')],
      ['self', ECHO_I18N.t('你的聲音'), ECHO_I18N.t('「救我……」——但你沒有說過這句話。')]
    ];
    const root = echoMiniGameShell('CH5 · RESIDUAL VOICES', ECHO_I18N.t('殘留訊息'), ECHO_I18N.t('這些不是即時訊息。點開三段殘留人格回聲。'), ECHO_MG_ASSETS.ch5.residualBg);
    const body = root.querySelector('.echo-mg-body');
    const foot = root.querySelector('.echo-mg-foot');
    const stage = document.createElement('div');
    stage.className = 'residual-stage';
    stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch5.residualBg, 'echo-mg-stage-img', ECHO_I18N.t('殘留訊息背景')));
    stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch5.residualOverlay, 'residual-overlay', ECHO_I18N.t('殘留訊息覆蓋層')));
    const note = document.createElement('div');
    note.className = 'echo-mg-note';
    note.textContent = ECHO_I18N.t('已讀取 0 / 3');
    ...
    voices.forEach((v, i) => {
      ...
      bubble.innerHTML = '<b>' + v[1] + ECHO_I18N.t('</b><span>點擊讀取殘留</span>');
      bubble.onclick = () => {
        if (opened.has(v[0])) return;
        opened.add(v[0]);
        bubble.classList.add('is-open');
        bubble.querySelector('span').textContent = v[2];
        note.textContent = ECHO_I18N.t('已讀取 ') + opened.size + ' / 3';
...
function runChoiceCandy() {
  return new Promise((resolve) => {
    let settled = false;
    const root = echoMiniGameShell('CH5 · CHOICE CANDY', ECHO_I18N.t('同步糖果'), ECHO_I18N.t('選擇一顆。'), ECHO_MG_ASSETS.ch5.candyScene);
    const body = root.querySelector('.echo-mg-body');
    const foot = root.querySelector('.echo-mg-foot');
    const stage = document.createElement('div');
    stage.className = 'echo-mg-stage candy-stage';
    stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch5.candyScene, 'echo-mg-stage-img', ECHO_I18N.t('紅藍糖果')));
    ...
    left.setAttribute('aria-label', ECHO_I18N.t('左側糖果'));
    right.setAttribute('aria-label', ECHO_I18N.t('右側糖果'));
    const note = document.createElement('div');
    note.className = 'echo-mg-note';
    note.textContent = ECHO_I18N.t('EVA：你可以自己選。');
    ...
    function choose(choice) {
      ...
      const highlight = echoArtImg(ECHO_MG_ASSETS.ch5.candyHighlight, 'candy-highlight', ECHO_I18N.t('選擇標記'));
      highlight.classList.add(choice === 'red' ? 'on-red' : 'on-blue');
      stage.appendChild(highlight);
      stage.appendChild(echoArtImg(ECHO_MG_ASSETS.ch5.candyResult, 'echo-mg-complete-art', ECHO_I18N.t('選擇結果')));
      note.textContent = choice === 'red' ? ECHO_I18N.t('你選擇保留現實。') : ECHO_I18N.t('你選擇接受同步。');
