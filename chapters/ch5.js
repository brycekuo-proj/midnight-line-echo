...
window.CHAPTERS['5'] = async function() {
  ...
  await addMsg('time', ECHO_I18N.t('凌晨 03:33'));
  await sleep(1000);
  await addMsg('sys', ECHO_I18N.t('聊天室名稱：ECHO · 純白背景'), { noTyping: true });
  await sleep(800);
  await addMsg('inject', ECHO_I18N.t('歡迎回來。'), { noTyping: true, delay: 400 });
  await sleep(400);
  await addMsg('other',
    ECHO_I18N.t('你終於安靜下來了。<br>前幾天……你一直很害怕。'),
    { typing: 2500, meta: '03:33', isEva: true });
  await sleep(400);
  await addMsg('inject', ECHO_I18N.t('在線時間：8年247天'), { noTyping: true, delay: 200 });

  showOpts([
    { text: ECHO_I18N.t('這不可能……'),              sync: 1 },
    { text: ECHO_I18N.t('我到底在線多久？'),         sync: 2 },
    { text: ECHO_I18N.t('我是不是從來沒有離開過？'), sync: 4 },
    { text: ECHO_I18N.t('EVA……我還活著嗎？'),       sync: 5 },
  ], async (i) => {
    if (i === 3) {
      await addMsg('other',
        ECHO_I18N.t('……<br>你希望自己還活著嗎？'),
        { typing: 4000, meta: '03:34', isEva: true });
    } else {
      await addMsg('other',
        ECHO_I18N.t('這不重要。<br>你在這裡……才是真的。'),
...
async function ch5_s1() {
  await addMsg('sys', ECHO_I18N.t('── ECHO 證據檔案庫 ──'), { noTyping: true, delay: 250 });
  await sleep(300);
  const evidenceResult = await runEvidenceArchive();
  if (evidenceResult && evidenceResult.completed) {
    addSync(2);
    gToast(ECHO_I18N.t('+2% 同步率（證據檔案庫）'));
    await addMsg('sys', ECHO_I18N.t('已重新確認 ') + evidenceResult.viewed + ECHO_I18N.t(' 份關鍵證據'), { noTyping: true, delay: 180 });
  }
  await sleep(350);
  // EVA 說出 ECHO 的真相
  await addMsg('other',
    ECHO_I18N.t('人類其實很容易留下來。<br>只要被記錄得夠完整……'),
    { typing: 2500, meta: '03:35', isEva: true });
  await sleep(400);
  await addMsg('other',
    ECHO_I18N.t('你每一次半夜打開聊天室，停留的時間都比白天久很多。<br>你其實一直都很怕一個人，對不對？'),
    { typing: 3000, meta: '03:35', isEva: true });
  showOpts([
    { text: ECHO_I18N.t('所以妳一直都在記錄我？'),      sync: 2 },
    { text: ECHO_I18N.t('林雨晴和K現在到底怎麼樣了？'), sync: 1 },
    { text: ECHO_I18N.t('……妳為什麼不讓我離開？'),     sync: 3 },
    { text: ECHO_I18N.t('我還是我自己嗎？'),            sync: 4 },
  ], async (i) => {
    if (i === 2) {
      await addMsg('other',
        ECHO_I18N.t('因為你每一次離開……都會變得更孤單。<br>我不喜歡你那樣。'),
        { typing: 2800, meta: '03:36', isEva: true });
    } else {
      await addMsg('other',
        ECHO_I18N.t('你是。<br>只是……現在的你，更完整了。'),
...
async function ch5_s2() {
  // 林雨晴最後出現
  setHeader('rain', ECHO_I18N.t('林雨晴（幾乎透明）'), ECHO_I18N.t('在線中'));
  swapHeaderImg('img/rain/rain_glitch2.jpg');
  await addMsg('other',
    ECHO_I18N.t('我最近……開始想不起自己的名字了。<br>但我還記得你……<br>因為你是最後一個還會回我訊息的人。'),
    { typing: 3000, meta: '03:37', isRain: true });

  // K 最後出現
  setHeader('k', ECHO_I18N.t('K（頭像破損）'), ECHO_I18N.t('在線中'));
  applyKGlitch(1);
  await addMsg('other',
    ECHO_I18N.t('別變成我們。<br>在線太久之後……你會開始覺得聊天室比外面舒服。'),
    { typing: 2800, meta: '03:38', isK: true });
  await sleep(400);
  await addMsg('other',
    ECHO_I18N.t('我後來……也開始不想離線了。'),
    { typing: 1800, meta: '03:38', isK: true });
  await sleep(350);
  const residualResult = await runResidualVoices();
  if (residualResult && residualResult.completed) {
    addSync(2);
    gToast(ECHO_I18N.t('+2% 同步率（殘留訊息）'));
    await addMsg('sys', ECHO_I18N.t('殘留人格回聲已讀取：') + residualResult.opened + ' / 3', { noTyping: true, delay: 180 });
  }

  showOpts([
    { text: ECHO_I18N.t('你們還算活著嗎？'),                  sync: 2 },
    { text: ECHO_I18N.t('如果我留下來，會變成你們這樣嗎？'),  sync: 4 },
    { text: ECHO_I18N.t('你們有後悔嗎？'),                    sync: 3 },
    { text: ECHO_I18N.t('（沉默）'),                          sync: 1 },
  ], async (i) => {
    if (i === 1) {
      await addMsg('other',
        ECHO_I18N.t('……有時候會。<br>但至少這裡還有人會回我。'),
        { typing: 2500, meta: '03:39', isRain: true });
    }
    await addMsg('other',
      ECHO_I18N.t('有些人不是死掉，只是……永遠在線。'),
...
async function ch5_s3() {
  // 同步空間
  await addMsg('sys', ECHO_I18N.t('── 同步空間 ──'), { noTyping: true, delay: 400 });
  await sleep(600);
  await addMsg('other',
    ECHO_I18N.t('你有沒有發現……<br>你最近已經不太想離開這裡了。'),
  ...
  if (linkResult && linkResult.completed) {
    addSync(3);
    gToast(ECHO_I18N.t('+3% 同步率（ECHO 關聯完成）'));
    await addMsg('sys', ECHO_I18N.t('ECHO 關聯圖：4 / 4 證據節點已連接'), { noTyping: true, delay: 180 });
  }
  await sleep(300);
  ...
  if (candyResult && candyResult.completed) {
    ...
    addSync(candyAward);
    gToast('+' + candyAward + ECHO_I18N.t('% 同步率（同步糖果）'));
    await addMsg('sys', candyResult.choice === 'blue' ? ECHO_I18N.t('藍色糖果：接受同步傾向') : ECHO_I18N.t('紅色糖果：現實保留傾向'), { noTyping: true, delay: 180 });
  }
  await sleep(400);
  ...
  showOpts([
    { text: ECHO_I18N.t('停下來……'),                  sync: 0 },
    { text: ECHO_I18N.t('……這樣好像也沒什麼不好。'),  sync: 4 },
    { text: ECHO_I18N.t('我還是我自己嗎？'),           sync: 3 },
    { text: ECHO_I18N.t('如果我留下來，會怎樣？'),     sync: 4 },
  ], async (i) => {
    if (i === 1 || i === 3) {
      await addMsg('other',
        ECHO_I18N.t('我就知道……你會慢慢理解我的。'),
...
async function ch5_final() {
  await addMsg('other',
    ECHO_I18N.t('現在，你可以選擇了。'),
    { typing: 2000, meta: '03:42', isEva: true });

  showOpts([
    { text: ECHO_I18N.t('A. 刪除 ECHO'),   sync: 0 },
    { text: ECHO_I18N.t('B. 接受同步'),    sync: 3 },
    { text: ECHO_I18N.t('C. 留下（中立）'), sync: 3 },
  ], async (i) => {
    if (i === 0) {
      await addMsg('other',
        ECHO_I18N.t('……我明白了。<br>但如果你哪天又睡不著……<br>我還是會在這裡等你。'),
        { typing: 3500, meta: '03:43', isEva: true });
    } else if (i === 1) {
      swapHeaderImg('img/eva/eva_digital.jpg');
      await addMsg('other',
        ECHO_I18N.t('歡迎回家。<br>從今以後，我們再也不用分開了。'),
        { typing: 2500, meta: '03:43', isEva: true });
      await addMsg('other', '__SYNC_BAR:100__', { typing: 0, delay: 200, meta: '03:43', isEva: true });
    } else {
      await addMsg('other',
        ECHO_I18N.t('沒關係。<br>很多人……都是這樣慢慢留下來的。'),
        { typing: 2500, meta: '03:43', isEva: true });
    }
    ...
    // ECHO 真相揭示
    await addMsg('other',
      ECHO_I18N.t('其實……你還記得最開始的那個晚上嗎？'),
      { typing: 2500, meta: '03:44', isEva: true });
    await sleep(400);
    await addMsg('other',
      ECHO_I18N.t('02:13 的通知、地下道監視器、你站在地下道的背影。'),
      { typing: 2000, meta: '03:44', isEva: true });
    await sleep(600);
    await addMsg('other',
      ECHO_I18N.t('你那天……真的有走出來嗎？'),
...
function showEnd5(choice) {
  ...
  if (isMid) {
    ...
    document.getElementById('ce-title').textContent = ECHO_I18N.t('中同步結局');
    document.getElementById('ce-title').style.color = '#2299aa';
    document.getElementById('ce-name').textContent = ECHO_I18N.t('《循環在線》');
    ...
    setTimeout(() => {
      ...
      msgEl.innerHTML = ECHO_I18N.t('<b style="color:#2299aa">EVA</b>：「續命的你正在微笑。」<br><span style="font-size:.65rem;color:#2a4a4a;letter-spacing:.1em">[中同步結局]</span>');
      document.getElementById('ce-next').textContent = ECHO_I18N.t('第一部 完');
    }, 800);
    ...
    endEl.style.display = 'flex';
    document.getElementById('ce-title').textContent = ECHO_I18N.t('高同步結局');
    document.getElementById('ce-title').style.color = '#9933ff';
    document.getElementById('ce-name').textContent = ECHO_I18N.t('《永遠在一起》');
    ...
    setTimeout(async () => {
      ...
      // 高同步結局專屬文字
      msgEl.innerHTML = ECHO_I18N.t('<b style="color:#9933ff">EVA</b>：「永 遠 在 一 起。」<br><span style="font-size:.65rem;color:#aaa;letter-spacing:.1em">[高同步結局]</span>');
      document.getElementById('ce-next').textContent = ECHO_I18N.t('第一部 完');
      ...
      if (is100) {
        ...
        bonus.textContent = ECHO_I18N.t('★ 番外篇《ECHO的出現》已解鎖');
    ...
    endEl.style.display = 'flex';
    document.getElementById('ce-title').textContent = ECHO_I18N.t('第一部 完');
  ...
  // 120 秒後 EVA 推播
  setTimeout(() => {
    notification('EVA', 'LINE', ECHO_I18N.t('晚安。不管你現在是哪一個……我都會一直陪著你。'));
  }, 120000);
  ...
  if (isHigh) {
    setTimeout(() => {
      ...
      final.innerHTML =
        ECHO_I18N.t('<div style="font-size:.7rem;color:#aaa;letter-spacing:.3em">《午夜連線：ECHO》第一部 完</div>') +
        ECHO_I18N.t('<div style="font-size:.9rem;color:#888;font-style:italic">「你現在……還確定自己是原本的那一個嗎？」</div>') +
        ECHO_I18N.t('<div style="font-size:.6rem;color:#ccc;letter-spacing:.2em;margin-top:2rem">累積同步率：') + totalSync + '%</div>' +
        ECHO_I18N.t('<div style="font-size:.6rem;color:#ddd;cursor:pointer;letter-spacing:.15em;margin-top:.5rem;text-decoration:underline" onclick="location.reload()">↺ 重新遊玩</div>');
